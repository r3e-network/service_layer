...
{
    public delegate void VoteBoostedHandler(UInt160 voter, string proposalId, BigInteger multiplier);

    [DisplayName("MiniAppGovBooster")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "Gov Booster - Governance voting power booster")]
    [ContractPermission("*", "*")]
    public class MiniAppGovBooster : SmartContract
    {
        ...
        public static void BoostVote(UInt160 voter, string proposalId, BigInteger multiplier)
        {
            ExecutionEngine.Assert(Runtime.CallingScriptHash == Gateway(), "only gateway");
            OnVoteBoosted(voter, proposalId, multiplier);
        }
