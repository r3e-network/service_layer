...
{
    public delegate void TurboResultHandler(UInt160 trader, bool won, BigInteger payout);

    [DisplayName("MiniAppTurboOptions")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "Turbo Options - Fast binary options trading")]
    [ContractPermission("*", "*")]
    public class MiniAppTurboOptions : SmartContract
    {
        ...
        public static void Resolve(UInt160 trader, bool won, BigInteger stake)
        {
            ExecutionEngine.Assert(Runtime.CallingScriptHash == Gateway(), "only gateway");
            OnTurboResult(trader, won, won ? stake * 185 / 100 : 0);
        }
