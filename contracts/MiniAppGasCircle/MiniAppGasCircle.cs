...
{
    public delegate void DepositHandler(UInt160 member, BigInteger amount);

    [DisplayName("MiniAppGasCircle")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "GAS Circle - Daily savings circle")]
    [ContractPermission("*", "*")]
    public class MiniAppGasCircle : SmartContract
    {
        ...
        public static void MakeDeposit(UInt160 member, BigInteger amount)
        {
            ExecutionEngine.Assert(Runtime.CheckWitness(member), "unauthorized");
            OnDeposit(member, amount);
        }
