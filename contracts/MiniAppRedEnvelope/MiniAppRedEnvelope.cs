...
{
    public delegate void EnvelopeClaimedHandler(UInt160 claimer, BigInteger amount);

    [DisplayName("MiniAppRedEnvelope")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "Red Envelope - Social GAS red packets")]
    [ContractPermission("*", "*")]
    public class MiniAppRedEnvelope : SmartContract
    {
        ...
        public static void Claim(UInt160 claimer, BigInteger amount)
        {
            ExecutionEngine.Assert(Runtime.CallingScriptHash == Gateway(), "only gateway");
            OnEnvelopeClaimed(claimer, amount);
        }
