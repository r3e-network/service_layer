...
{
    public delegate void ILCompensatedHandler(UInt160 provider, BigInteger compensation);

    [DisplayName("MiniAppILGuard")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "IL Guard - Impermanent loss protection")]
    [ContractPermission("*", "*")]
    public class MiniAppILGuard : SmartContract
    {
        ...
        public static void Compensate(UInt160 provider, BigInteger compensation)
        {
            ExecutionEngine.Assert(Runtime.CallingScriptHash == Gateway(), "only gateway");
            OnILCompensated(provider, compensation);
        }
