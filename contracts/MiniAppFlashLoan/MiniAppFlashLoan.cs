...
{
    public delegate void LoanExecutedHandler(UInt160 borrower, BigInteger amount, BigInteger fee);

    [DisplayName("MiniAppFlashLoan")]
    [ManifestExtra("Author", "R3E Network")]
    [ManifestExtra("Version", "1.0.0")]
    [ManifestExtra("Description", "Flash Loan - Instant borrow and repay")]
    [ContractPermission("*", "*")]
    public class MiniAppFlashLoan : SmartContract
    {
        ...
        public static void ExecuteLoan(UInt160 borrower, BigInteger amount)
        {
            ExecutionEngine.Assert(Runtime.CheckWitness(borrower), "unauthorized");
            BigInteger fee = amount * 9 / 10000;
            OnLoanExecuted(borrower, amount, fee);
        }
