...
export async function executeVerifiedCompute(
  ...
  if (!response.ok) {
    const fallbackMessage = t ? t("computeFailed", { status: response.status }) : `Compute failed: ${response.status}`;
    const error = await response.json().catch(() => ({ message: "" }));
    const message = t ? fallbackMessage : error.message || fallbackMessage;
 ...
 */
export function useHybridCompute(t?: Translate) {
  ...
  async function executeHybrid<TInitResult, TComputeResult, TSettleResult>(
    ...
    try {
      ...
      const computeResponse = await executeVerifiedCompute(computeParams, authToken, t);

      if (!computeResponse.success || !computeResponse.verification.verified) {
        throw new Error(t ? t("computeVerificationFailed") : "Computation verification failed");
      }
      ...
      const fallbackMessage = t ? t("unknownError") : "Unknown error";
      const message = e instanceof Error ? e.message : fallbackMessage;
      computeError.value = message || fallbackMessage;
      throw e;
    } finally {
