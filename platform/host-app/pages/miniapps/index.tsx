...
export default function MiniAppsPage() {
  ...
  const [apps] = useState<MiniAppInfo[]>(baseApps);
  ...
  }, [filters, sortBy, viewMode, isUrlInitialized, router]);
  ...
  const filteredAndSortedApps = useMemo(() => {
    ...
    if (filters.chains?.length) {
      result = result.filter((app) => {
        ...
        return filters.chains.some((c: string) => appChains.includes(c as ChainId));
      });
    }
    ...
    return result;
  }, [apps, communityApps, statsMap, searchQuery, filters, sortBy, collectionsSet, locale]);
