...
export default function ContainerPage({ appId, app }: ContainerPageProps) {
  ...
  const { chainId: storeChainId } = useWalletStore();
  ...
  const iframeSrc = useMemo(() => {
    ...
    return buildMiniAppEntryUrl(entryUrl, {
      lang: getMiniappLocale(locale),
      theme,
      embedded: "1",
      container: "true",
      layout,
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (data.type === "miniapp_ready") {
        ...
        if (sdk?.getConfig) {
          iframeRef.current.contentWindow?.postMessage({ type: "miniapp_config", config: sdk.getConfig() }, origin);
        }
        return;
      }
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black">
        <div className="text-center text-white">
          ...
          <button onClick={handleClose} className="mt-4 px-4 py-2 bg-neo text-white rounded">
            Close
          </button>
        </div>
      </div>
...
async function dispatchBridgeCall(
  sdk: ReturnType<typeof installMiniAppSDK>,
  method: string,
  params: unknown[],
