...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    switch (req.method) {
        case "POST":
            return handleCreate(req, res, appId, walletAddress);
        case "GET":
            return handleGet(req, res, appId);
        case "PUT":
            return handleUpdate(req, res, appId, walletAddress);
        default:
            return res.status(405).json({ error: "Method not allowed" });
    }
}

async function handleCreate(req: NextApiRequest, res: NextApiResponse, appId: string, creator: string) {
    ...
    const signerAccounts = transaction.signers.map((signer: { account: { toBigEndian: () => string } }) => signer.account.toBigEndian().toLowerCase());
...
async function handleGet(req: NextApiRequest, res: NextApiResponse, appId: string) {
    const { id } = req.query;
    const walletAddress = req.headers["x-wallet-address"] as string;
...
async function handleUpdate(req: NextApiRequest, res: NextApiResponse, appId: string, signerAddress: string) {
