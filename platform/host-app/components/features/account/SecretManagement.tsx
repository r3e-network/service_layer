...
function SecretItem({ secret, onDelete }: { secret: Secret; onDelete: (id: number) => void }) {
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-dark-900/50 border border-gray-200 dark:border-white/5 transition-colors hover:bg-gray-100 dark:hover:bg-dark-900/80">
      ...
      <Button variant="ghost" size="sm" onClick={() => onDelete(secret.id)} className="text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10">
        <Trash2 size={16} />
      </Button>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-dark-900 rounded-2xl p-6 max-w-md w-full border border-gray-200 dark:border-white/10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5 block">{t("account.secrets.name")}</label>
            <Input
              value={name}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5 block">{t("account.secrets.value")}</label>
            <Input
              value={value}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5 block">{t("account.secrets.description")}</label>
            <Input
              value={description}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5 block">{t("account.secrets.password")}</label>
            <Input
              value={password}
