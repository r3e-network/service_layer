...
export function ConnectButton() {
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-3 bg-white dark:bg-white/10 px-4 py-2 border border-gray-200 dark:border-white/10 rounded-full shadow-sm hover:border-gray-300 dark:hover:border-white/20 transition-all duration-300 group">
          <div className={`h-2 w-2 rounded-full ${isSocial ? "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]" : "bg-neo shadow-[0_0_8px_rgba(0,229,153,0.6)]"}`} />
          ...
          <span className="text-xs text-gray-600 dark:text-gray-300 font-medium tabular-nums">{displayBalance} GAS</span>
          <button
        ...
        {!isSocial && (
          <Button variant="ghost" size="sm" onClick={disconnect} className="text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-full px-4">
            {t("wallet.disconnect")}
          </Button>
        )}
      </div>
  ...
  return (
    <div className="relative">
      ...
      >
        {loading ? (
          <span className="flex items-center gap-2"><RefreshCw size={14} className="animate-spin" /> {t("wallet.connecting")}</span>
        ) : isSocialAuthActive ? (
          <span className="flex items-center gap-2"><User size={16} /> {t("wallet.socialLinked")}</span>
        ) : (
          <span className="flex items-center gap-2">{t("wallet.connect")} <ChevronDown size={14} className={`transition-transform duration-200 ${showMenu ? "rotate-180" : ""}`} /></span>
        )}
      </Button>
      ...
      {error && (
        <div className="absolute right-0 top-full mt-2 w-72 rounded-xl border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-900/90 backdrop-blur-md p-4 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200 z-40">
          <div className="flex gap-3">
            <div className="mt-0.5 text-red-500"><div className="w-2 h-2 rounded-full bg-current shadow-[0_0_8px_currentColor]" /></div>
            <div className="flex-1">
              <p className="text-sm text-red-600 dark:text-red-200 font-bold leading-tight">{error}</p>
              <button onClick={clearError} className="mt-2 text-xs text-red-500 hover:text-red-700 dark:hover:text-red-100 font-bold uppercase tracking-wide">
                {t("actions.dismiss")}
              </button>
            </div>
          </div>
