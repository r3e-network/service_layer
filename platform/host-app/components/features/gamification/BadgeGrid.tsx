...
const rarityColors = {
  common: "bg-erobo-purple/10 dark:bg-erobo-bg-card border-erobo-purple/20",
  ...
  return (
    <button
      ...
      className={`aspect-square p-2 rounded-none border-2 flex items-center justify-center transition-all shadow-brutal-xs hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] ${earned
          ...
          : "bg-erobo-purple/5 dark:bg-black border-erobo-purple/15 dark:border-white/10 opacity-50 grayscale"
        }`}
      title={badge.name}
    >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="text-center pt-2">
          ...
          <span
            className={`inline-block mt-2 px-3 py-1 text-xs font-black uppercase border-2 border-black dark:border-white rounded-none shadow-brutal-xs ${rarityColors[badge.rarity] || "bg-erobo-purple/10"}`}
          >
            {badge.rarity}
          </span>

          <p className="mt-4 text-sm font-bold text-erobo-ink-soft dark:text-slate-300 uppercase leading-snug">{badge.description}</p>
          <p className="mt-2 text-xs font-mono text-erobo-ink-soft dark:text-slate-500 bg-erobo-purple/10 dark:bg-black border border-erobo-purple/15 dark:border-white/10 p-2 break-all">{badge.requirement}</p>
