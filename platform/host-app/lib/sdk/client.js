/**
 * MiniApp SDK Client
 * Implements bridge methods for Neo N3 providers.
 */
...
function inferChainType(chainId, config) {
  const requested = config?.chainType;
  if (requested && requested !== "neo-n3") {
    throw new Error(`Unsupported chainType: ${requested}`);
  }
  if (chainId) {
    const registry = getChainRegistry();
    if (!registry.getChain(chainId)) {
...
export function createMiniAppSDK(config) {
  ...
  async function invokeWithWallet(invocation) {
    ...
    if (invocationType && invocationType !== "neo-n3") {
  ...
  return {
    ...
    invoke: async (method, params) => {
      ...
      if (method === "invokeFunction" && params) {
        ...
        if (targetChainType && targetChainType !== "neo-n3") {
          throw new Error("Unsupported chain type");
        }
