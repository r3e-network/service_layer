...
/** Get DB client — all rollup functions guard with isSupabaseConfigured first */
function db() {
  if (!supabaseAdmin) throw new Error("Database not configured");
  return supabaseAdmin;
}
import { getBlockCount } from "../chains/rpc-functions";
...
async function getRollupContext(chainId: ChainId, logId: number): Promise<RollupContext> {
  ...
  const { data: lastRollup } = await db()
    .from("stats_rollup_log")
...
async function updateAppStats(appId: string, events: ContractEventRow[], _context: RollupContext): Promise<void> {
  ...
  // Get existing stats
  const { data: existing } = await db().from("miniapp_stats").select("*").eq("app_id", appId).single();
  ...
  if (existing) {
    ...
    await db().from("miniapp_stats").insert({
      app_id: appId,
...
export async function getRollupStatus(): Promise<{
  ...
  const { count: totalRollups } = await db().from("stats_rollup_log").select("*", { count: "exact", head: true });

  const { count: failedRollups } = await db()
    .from("stats_rollup_log")
...
export async function resetRollup(): Promise<void> {
  ...
  await db().from("stats_rollup_log").delete().neq("id", 0);
}

export { ROLLUP_INTERVAL_MS, BLOCKS_PER_ROLLUP, DEFAULT_CHAIN_ID, type RollupContext, type RollupResult };
