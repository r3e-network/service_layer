...
export function hasPermission(method: string, permissions: MiniAppInfo["permissions"]): boolean {
  ...
  if (requiredPerm === undefined) return false; // Unknown method - deny
...
const handleSwitchChain: BridgeHandler = async ({ sdk, params }) => {
  ...
  if (sdk.wallet?.switchChain) return sdk.wallet.switchChain(chainId);
  const { useWalletStore } = await import("./wallet/store");
  await useWalletStore.getState().switchChain(chainId as string);
  return true;
};
...
const handleShareGetUrl: BridgeHandler = async ({ sdk, params, appId }) => {
  ...
  return buildShareUrl(appId, chainId, shareOptions);
};
...
const handleShareCopy: BridgeHandler = async ({ sdk, params, appId }) => {
  ...
  const shareOptions = options && typeof options === "object" ? options as { page?: string; params?: Record<string, string> } : {};
  const chainId = sdk.getConfig?.()?.chainId;
  const url = buildShareUrl(appId, chainId, shareOptions);
  try {
