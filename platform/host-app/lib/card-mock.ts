/**
 * Mock Card Data Generators for Development
 */
import type { CountdownData, MultiplierData, CanvasData, StatsData, VotingData, PriceData } from "@/types/card-display";

export function mockCountdownData(): CountdownData {
  const now = Date.now();
  const endTime = now + 3600000 + Math.random() * 7200000;
  return {
    type: "live_countdown",
    refreshInterval: 1,
