...
export async function getNeoBurgerStats(network: "mainnet" | "testnet" = "mainnet"): Promise<NeoBurgerStats> {
  ...
  try {
    ...
    const [supplyResult, gasPerNeoResult, prices] = await Promise.all([
      invokeRead(rpcUrl, NEOBURGER_CONTRACT, "totalSupply", []),
      invokeRead(rpcUrl, NEOBURGER_CONTRACT, "getGasPerNeo", []),
    ...
    // NeoBurger APY is typically around 15-20%.
    // GAS Reward ~ 0.3 - 0.4 per NEO per year with voting?

    // Let's update `annualGasRate` to a more realistic standard value for optimized voting:
    // ~ 0.35 GAS per NEO per year.
    const optimizedGasPerNeoPerYear = 0.35;

    const apr = ((optimizedGasPerNeoPerYear * prices.gas) / prices.neo) * 100;
