...
jest.mock("../../components/LaunchDock", () => ({
  LaunchDock: ({ appName, onExit, onShare }: { appName: string; onExit: () => void; onShare: () => void }) => (
...
describe("LaunchPage", () => {
  ...
  let mockSDK: {
    ...
    rng: {
      requestRandom: jest.Mock;
    };
    datafeed: {
      getPrice: jest.Mock;
    };
    stats: {
      getMyUsage: jest.Mock;
    };
    events: {
      list: jest.Mock;
    };
  ...
  beforeEach(() => {
    ...
    (window as unknown as { NEOLineN3: { Init: jest.Mock } }).NEOLineN3 = {
  ...
  describe("Wallet Connection", () => {
    it("does not auto-connect wallet on mount", async () => {
      ...
      expect((window as unknown as { NEOLineN3: { Init: jest.Mock } }).NEOLineN3.Init).not.toHaveBeenCalled();
    });

    it("renders even when a wallet provider is present", async () => {
      (window as unknown as { NEOLineN3: { Init: jest.Mock } }).NEOLineN3.Init = jest.fn().mockImplementation(() => ({
  ...
  describe("MiniApp SDK bridge", () => {
    ...
    it("injects MiniAppSDK into same-origin sandboxed iframes", async () => {
      ...
      expect((contentWindow as { MiniAppSDK?: unknown }).MiniAppSDK).toBeDefined();
...
describe("getServerSideProps", () => {
  ...
  it("should return app props for valid app_id", async () => {
    ...
    expect((result as { props: { app: { app_id: string } } }).props.app.app_id).toBe("test-app");
    expect((result as { props: { app: { name: string } } }).props.app.name).toBe("Test App");
  ...
  it("should return app with required fields", async () => {
    ...
    const app = (result as { props: { app: Record<string, unknown> } }).props.app;
