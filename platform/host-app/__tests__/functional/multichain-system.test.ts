...
describe("Multi-Chain Support", () => {
  describe("Chain Contract Lookup", () => {
    ...
    it("should return contract address for supported chain", () => {
      const appWithContracts = BUILTIN_APPS.find(
        (a) => a.chainContracts && Object.keys(a.chainContracts).length > 0
      );
      if (appWithContracts) {
        const chainId = Object.keys(appWithContracts.chainContracts!)[0];
        const contract = getContractForChain(appWithContracts, chainId as any);
  ...
  describe("Chain Support Check", () => {
    it("should return true for explicitly supported chains", () => {
      const appWithChains = BUILTIN_APPS.find(
        (a) => a.supportedChains && a.supportedChains.length > 0
      );
      if (appWithChains) {
    ...
    it("should return false for unsupported chains", () => {
      const app = BUILTIN_APPS[0];
      expect(isChainSupported(app, "fake-chain-id" as any)).toBe(false);
    });
  });
  ...
  describe("Get All Supported Chains", () => {
    ...
    it("should return empty array for apps without chain support", () => {
      const mockApp: MiniAppInfo = {
        ...
        supportedChains: [],
        status: null,
        permissions: { payments: false, governance: false, rng: false, datafeed: false, confidential: false, automation: false },
        limits: null,
  ...
  describe("Chain ID Resolution", () => {
    ...
    it("should fallback to first supported chain", () => {
      ...
      if (appWithChains) {
        const result = resolveChainIdForApp(appWithChains, "unsupported-chain" as any);
