...
Deno.serve(async (req) => {
  ...
  const scopeCheck = requireScope(auth, "automation-triggers");
  ...
  if (req.method === "GET") {
    const result = await getJSON(`${neoflowURL}/triggers`, { "X-User-ID": auth.userId });
  ...
  const result = await postJSON(`${neoflowURL}/triggers`, body, { "X-User-ID": auth.userId });
  if (result instanceof Response) return result;
  return json(result);
});
