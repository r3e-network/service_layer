...
The codebase contains an optional Chainlink reader (feeds read via Arbitrum
RPC). It is **disabled by default** to keep default behavior aligned with the
platform blueprint (3 HTTP sources + median). To enable it, set `ARBITRUM_RPC`
for the `neofeeds` marble and pass it via `Config.ArbitrumRPC`.
