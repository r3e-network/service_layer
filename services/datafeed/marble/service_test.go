...
func TestServiceConstants(t *testing.T) {
	if ServiceID != "neofeeds" {
		t.Errorf("ServiceID = %s, want neofeeds", ServiceID)
	}
	if ServiceName != "NeoFeeds Service" {
		t.Errorf("ServiceName = %s, want NeoFeeds Service", ServiceName)
	}
	if Version != "3.0.0" {
