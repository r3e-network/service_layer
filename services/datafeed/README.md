...
Optional: if `ARBITRUM_RPC` is configured, Chainlink is enabled as an additional data source (feeds read via Arbitrum RPC; it does not replace HTTP sources).
...
| OKX | HTTP API | Yes | 1 |
| Chainlink (via Arbitrum RPC) | On-chain | Optional | 1 |
...
|----------|-------------|
| `ARBITRUM_RPC` | Arbitrum RPC URL for Chainlink price feeds |
