# NeoVRF On‑Chain Contract

NeoVRF (`service_id`: `neovrf`) anchors randomness outputs on-chain using the
platform `RandomnessLog` contract.
...
- Each record stores the randomness value and an `attestation_hash`.
