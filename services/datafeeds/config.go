// Package datafeeds provides configurable price feed aggregation service.
package datafeeds
...
// DataFeedsConfig is the root configuration for the datafeeds service.
...
func LoadConfigFromFile(path string) (*DataFeedsConfig, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("read config file: %w", err)
	}

	var cfg DataFeedsConfig
...
func (c *DataFeedsConfig) Validate() error {
	if len(c.Sources) == 0 {
...
func (c *DataFeedsConfig) GetSource(id string) *SourceConfig {
	for i := range c.Sources {
...
func (c *DataFeedsConfig) GetFeed(id string) *FeedConfig {
	for i := range c.Feeds {
		if c.Feeds[i].ID == id {
			return &c.Feeds[i]
		}
...
func (c *DataFeedsConfig) GetEnabledFeeds() []FeedConfig {
...
func DefaultConfig() *DataFeedsConfig {
	return &DataFeedsConfig{
		Version: "1.0",
...
func (c *DataFeedsConfig) ToJSON() ([]byte, error) {
	return json.MarshalIndent(c, "", "  ")
}

// ToYAML serializes config to YAML.
func (c *DataFeedsConfig) ToYAML() ([]byte, error) {
