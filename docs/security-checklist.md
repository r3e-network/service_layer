...
- Governance: **NEO only**
- Explicitly reject: any other assets for payment/governance paths.
...
- Omit `allow-same-origin` for untrusted MiniApps; handle `origin === "null"` safely.
- Set `referrerPolicy="no-referrer"` on sandboxed MiniApp iframes.
