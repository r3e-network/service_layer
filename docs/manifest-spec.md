...
This repository targets a **Neo N3 MiniApp Platform**. MiniApps are loaded by a host application (Next.js) and interact with the Neo N3 mainnet/testnet through the platform SDK + attested services.
...
```json
{
    ...
    "icon": "/miniapps/neo-game/static/logo.jpg",
    "banner": "/miniapps/neo-game/static/banner.jpg",
    "category": "gaming",
...
- `supported_chains` (array, required): list of chain IDs the MiniApp supports
  (e.g. `neo-n3-mainnet`, `neo-n3-testnet`). Chain IDs must be configured in the
  platform chain registry.
