...
**用户永远不会直接连接到任何已知的服务层账户。** 这通过以下方式实现：

1. **直接池存款**: 用户直接存款到匿名池账户（不是 gasbank，不是任何已知的服务层地址）
2. **隐式费用收取**: 费用从交付金额中扣除（用户收到 `净金额 = 总金额 - 服务费`）
3. **无费用交易**: 服务费保留在池账户中分布 - 没有显式的费用转账到任何已知地址
4. **零链上关联**: 没有交易将用户连接到服务层身份

```
用户存款:  100 GAS  →  池账户 (匿名)
用户收到:  99.5 GAS →  目标地址
费用保留:  0.5 GAS  →  留在池中 (无显式转账)
```
...
# 查看日志
docker compose -f docker/docker-compose.simulation.yaml logs -f gateway
```
...
│   │  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐  │    │
│   │  │ VRF_PRIVATE_KEY  │  │ NEOVAULT_MASTER_KEY │  │ NEOFEEDS_SIGNING_KEY │  │  │
...
✅ services/neoaccounts    - (内部服务，测试已通过)
