...
- Config snippet (YAML):
  ...
  ```
- CLI (prototype, requires JAM_ENABLED=1): `slctl jam preimage --file <path> [--hash <sha256>]`, `slctl jam package --service <id> --kind <k> --params-hash <h>`, `slctl jam packages` (list), `slctl jam process`, `slctl jam report --package <id>`.
