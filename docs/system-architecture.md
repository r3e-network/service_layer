...
- **Application Services**: Modular domains under `packages/com.r3e.services.*` (accounts, automation, gasbank, oracle, datafeeds, datalink, datastreams, DTA, CRE, CCIP, VRF, random, confidential, JAM). The handler (`applications/httpapi`) routes requests and applies auth/tenant checks.
...
- **Persistence**: Self-hosted Supabase Postgres (compose service `supabase-postgres` via the Supabase profile) driven by embedded migrations in `system/platform/migrations`; `DATABASE_URL` is the canonical DSN override across flags/env/config. In-memory stores exist for tests only.
