...
- **Persistence**: Self-hosted Supabase Postgres (compose service `supabase-postgres` via the Supabase profile) driven by embedded migrations in `internal/platform/migrations`; `DATABASE_URL` is the canonical DSN override across flags/env/config. In-memory stores exist for tests only.
