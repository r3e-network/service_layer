...
export function useTurtleGame(APP_ID: string) {
  ...
  const loadStats = async () => {
    try {
      ...
      const [sessionsRes, paidRes] = await Promise.all([
        invokeRead({ contractAddress: contract, operation: "totalSessions", args: [] }),
        invokeRead({ contractAddress: contract, operation: "totalPaid", args: [] }),
      ]);
  ...
  const settleGame = async (): Promise<boolean> => {
    ...
    try {
      ...
      error.value = e.message || "Failed to settle game";
      return false;
    } finally {
      loading.value = false;
    }
