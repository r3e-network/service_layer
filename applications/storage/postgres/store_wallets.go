...
func (s *Store) CreateWorkspaceWallet(ctx context.Context, wallet account.WorkspaceWallet) (account.WorkspaceWallet, error) {
	...
	tenant := s.accountTenant(ctx, wallet.WorkspaceID)

	_, err := s.db.ExecContext(ctx, `
		INSERT INTO workspace_wallets (id, workspace_id, wallet_address, label, status, tenant, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
	`, wallet.ID, wallet.WorkspaceID, wallet.WalletAddress, wallet.Label, wallet.Status, tenant, wallet.CreatedAt, wallet.UpdatedAt)
	if err != nil {
		return account.WorkspaceWallet{}, err
	}
	return wallet, nil
...
func (s *Store) ListWorkspaceWallets(ctx context.Context, workspaceID string) ([]account.WorkspaceWallet, error) {
	tenant := s.accountTenant(ctx, workspaceID)
	query := `
		SELECT id, workspace_id, wallet_address, label, status, created_at, updated_at
		FROM workspace_wallets
