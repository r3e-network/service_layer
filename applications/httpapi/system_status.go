...
// systemTenant echoes the tenant derived from headers/JWT for quick diagnostics.
// It also reports whether REQUIRE_TENANT_HEADER is enforced and whether a tenant
// claim was mapped from JWTs. Useful for operators validating Supabase claims.
func (h *handler) systemTenant(w http.ResponseWriter, r *http.Request) {
	...
	resp := map[string]any{
		...
		"user":                     user,
		"role":                     role,
		"require_tenant_header_on": requireTenantHeaderEnabled(),
	}
